...
export const createMultipleTestAccounts = async (
  ...
  console.log(`✅ Created ${results.filter(r => r.success).length}/${accounts.length} test accounts`);
...
export const runAuthSystemTest = async (): Promise<{ success: boolean; results: any[] }> => {
  ...
  if (!envValidation.isValid) {
    console.error('❌ Environment not properly configured:', envValidation.issues);
