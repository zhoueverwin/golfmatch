...
const EditProfileScreen: React.FC = () => {
  ...
  const openCamera = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        const newProfilePictures = [result.assets[0].uri, ...formData.profile_pictures.slice(1)];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await DataProvider.updateUserProfile('current_user', updateData);
      ...
      >
        ...
        <Card style={styles.photoCard} shadow="small">
          <View style={styles.photoSection}>
            <Image
              source={{ 
                uri: formData.profile_pictures.length > 0 
        ...
        <Card style={styles.sectionCard} shadow="small">
          ...
          {renderSelectField('血液型', 'blood_type', ['A型', 'B型', 'O型', 'AB型'])}
          {renderInputField('身長 (cm)', 'height', '身長を入力してください')}
          
          {renderSelectField('体型', 'body_type', ['やせ型', '普通', 'ぽっちゃり', '筋肉質'])}
          {renderSelectField('タバコ', 'smoking', ['吸わない', '吸う', '時々吸う'])}
        </Card>
