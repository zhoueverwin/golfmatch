...
const PostCreationModal: React.FC<PostCreationModalProps> = ({
  ...
  const handlePublish = async () => {
    ...
    if (invalidVideos.length > 0) {
      console.error("Invalid video URLs detected:", invalidVideos);
      Alert.alert(
        "動画エラー",
        "無効な動画ファイルが含まれています。別の動画を選択してください。",
      );
      return;
    }
    ...
    try {
      ...
      if (images.length > 0) {
        ...
        // Include already uploaded images (from editing)
        uploadedImageUrls = [...uploadedImageUrls, ...remoteImages];
      }

      // Upload video to Supabase Storage if it's a local file
      if (videos.length > 0) {
        ...
        if (videoUri.startsWith("file://")) {
          ...
          // Already uploaded video (from editing)
      ...
      // Publish with Supabase Storage URLs
      await onPublish({
        text: text.trim(),
          ...
          >
            ...
            <View style={styles.mediaButtons}>
              <TouchableOpacity
                style={[
                  styles.mediaButton,
                  videos.length > 0 && styles.mediaButtonDisabled,
                ]}
              ...
              >
                ...
                <Text
                  style={[
                    ...
                    videos.length > 0 && styles.mediaButtonTextDisabled,
                  ]}
                >
                  写真
                </Text>
